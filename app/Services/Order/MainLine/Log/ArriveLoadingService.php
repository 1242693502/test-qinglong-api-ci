...
class ArriveLoadingService extends BaseService
{
    /**
     * 到达装货地
     *
    ...
    {
        // 检查是否允许操作
        $this->checkActionAllow($orderUUID, 'arrive_loading');
        ...
        $mainLinePlace = MainLine\Place::where('place_uuid', $placeUUID)->where('type',
            cons('order.mainline.place.type.loading'))->first(['id', 'area_name', 'address', 'order_uuid']);
        if (!$mainLinePlace) {
            throw new Client\BadRequestException('装货地址不存在');
        }
        ...
        // 验证订单是否能够切换状态
        if (!((new StatusService())->canChangeStatus($orderMainLine->order_status, 'arrive_loading'))) {
            throw new Client\ForbiddenException('当前订单状态切换无效');
        }

        //改变订单状态
        (new StatusService())->changeStatus($orderMainLine, 'arrive_loading');
        ...
        //记录$parameters
        $description = '到达装货地：' . $data['full_address'];
        $images      = [];

        $orderMainLineLog = (new LogService())->logArriveLoading($orderMainLine, $driverUUID,
