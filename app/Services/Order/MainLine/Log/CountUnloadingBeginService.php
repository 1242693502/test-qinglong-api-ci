...
class CountUnloadingBeginService extends BaseService
{
    /**
     * 记录卸货计时开始
     *
    ...
    public function create($orderUUID, $driverUUID, $parameters = [])
    {
        ...
        // 记录卸货开始计时时间
        $mainLinePlace->setAttribute('count_begin_time', Carbon::now())->save();
        ...
        //记录$parameters
        $description      = '卸货计时开始，卸货地：' . $data['full_address'];
        $orderMainLineLog = (new LogService())->logCountUnloadingBegin($orderMainLine, $driverUUID,
