...
class NoCountUnloadingService extends BaseService
{
    /**
     * 甩挂无需计时
     *
    ...
    public function create($orderUUID, $driverUUID, $parameters = [])
    {
        ...
        // 检查是否允许操作
        $this->checkActionAllow($orderUUID, 'count_unloading_begin');
        ...
        // 设置已完成装货开始计时
        ActionService::serviceForOrderUUID($orderMainLine->order_uuid)->setActionsDone('count_unloading_begin');
        ...
        //记录$parameters
        $description      = '甩挂无需计时，卸货地：' . $data['full_address'];
        $orderMainLineLog = (new LogService())->logCountUnloadingEnd($orderMainLine, $driverUUID,
